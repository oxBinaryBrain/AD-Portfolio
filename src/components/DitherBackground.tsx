...
float pattern(vec2 p) {
  vec2 movement = vec2(time * waveSpeed * 0.7, time * waveSpeed * 0.5);
  vec2 p2 = p - movement;
  float n = fbm(p2);
  return fbm(p + n + movement * 0.3);
}
...
function WavePlane({
  ...
  const material = useMemo(
    ...
    [waveAmplitude, waveColor, waveFrequency, waveSpeed, enableMouseInteraction, mouseRadius],
  );
...
export function DitherBackground(props: {
  waveSpeed?: number;
  waveFrequency?: number;
  waveAmplitude?: number;
  waveColor?: [number, number, number];
